...
gets(v.voting_position); printf("\nEnter
Your Age: "); scanf("%d",&v.age);
fflush(stdin);
 //check age of voter
if(v.age < 18){
...
while(fscanf(fp,"%[^,],%[^,],%[^,],%[^,],%[^,],%[^,],%d,%d\n",v.name,v.dob,v.address,v.voting_area,v.voting_position,v.id,&v.
age,&v.voted)!=EOF){
 if(strcmp(v.id,id)==0){
 found = 1;
 break;
